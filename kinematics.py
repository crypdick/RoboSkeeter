...
class Kinematics(object):
    # TODO: move io funcs to new script
    # TODO: make kinematic math scripts
    # TODO: move visualization helper funcs to new script
    def __init__(self, experiment):
        ...
        self.is_agent = experiment.is_experiment
        self.plume = experiment.environment.plume
        self.windtunnel = experiment.environment.windtunnel
    ...
    def calc_kinematic_vals(self):
        self.kinematics['curvature'] = calculate_curvature(self.kinematics)
        # absolute magnitude of velocity, accel vectors in 3D
        self.kinematics['velocity_magn'] = np.linalg.norm(self.kinematics.loc[:, ('velocity_x', 'velocity_y', 'velocity_z')],
                                                          axis=1)
        self.kinematics['acceleration_magn'] = np.linalg.norm(
            self.kinematics.loc[:, ('acceleration_x', 'acceleration_y', 'acceleration_z')], axis=1)
        self.kinematics['dist_to_wall'] = distance_from_wall(self.kinematics[['position_x', 'position_y', 'position_z']],
    ...
    def plot_door_velocity_compass(self, region='door', kind='avg_mag_per_bin'):
        ...
        if region == 'door':
            """plot the area """
            ensemble = self.kinematics.loc[
                ((self.kinematics['position_x'] > 0.25) & (self.kinematics['position_x'] < 0.5)), ['velocity_x', 'velocity_y']]
        else:
            ensemble = self.kinematics
    ...
    def plot_plume_triggered_compass(self, kind='avg_mag_per_bin'):
        ...
        for behavior in behaviors:
            ensemble = self.kinematics.loc[self.kinematics.plume_experience == behavior]
    ...
    def plot_sliced_hists(self):
        ...
        print "full ensemble"
        full_ensemble = self.kinematics.loc[(self.kinematics['position_x'] > x_edges[0]) \
                                            & (self.kinematics['position_x'] < x_edges[4])]
        self.plot_kinematic_hists(ensemble=full_ensemble, titleappend=' {} < x <= {}'.format(x_edges[0], x_edges[4]))

        print "downwind half ensemble"
        downwind_ensemble = self.kinematics.loc[(self.kinematics['position_x'] > x_edges[0]) \
                                                & (self.kinematics['position_x'] < x_edges[2])]
        self.plot_kinematic_hists(ensemble=downwind_ensemble,
                                  titleappend=' {} < x <= {}'.format(x_edges[0], x_edges[2]))

        print "upwind half ensemble"
        upwind_ensemble = self.kinematics.loc[(self.kinematics['position_x'] > x_edges[2]) \
                                              & (self.kinematics['position_x'] < x_edges[4])]
    ...
    def plot_timeseries(self, kinematic=None):
        ...
        import scripts.plotting_sorcery
        scripts.plotting_sorcery.plot_timeseries(self.kinematics, self.agent_obj, kinematic=kinematic)

    def plume_stats(self):
        """ 
        in plume == 1, out == 0. therefore sum/n is % in plume
        """
        in_out = self.kinematics.inPlume.values
        N_timesteps = in_out.size
        N_timesteps_in_plume = in_out.sum()

        percent_time_in_plume = 1.0 * N_timesteps_in_plume / N_timesteps
        print "Percent time in plume = {}".format(percent_time_in_plume)
    ...
    def describe(
            ...
            plot_kwargs={
                ...
                'force_violin': True}):
        print self.kinematics.describe()
    ...
    def visualize_forces(self):
        ...
        if self.is_agent is True:
            import scripts.plotting_sorcery
            scripts.plotting_sorcery.plot_all_force_clouds(self.kinematics)
        else:
    ...
    def plot_force_violin(self):
        if self.is_agent is True:
            import scripts.plotting_sorcery
            scripts.plotting_sorcery.plot_forces_violinplots(self.kinematics, self.agent_obj)
        else:
            raise TypeError("Can't visualize the forces for an experimental trajectory because we don't have that information.")
    ...
    def plot_start_postiions(self):
        import scripts.plotting_sorcery
        scripts.plotting_sorcery.plot_starting_points(self.kinematics)
    ...
    def load_experiments(self, experimental_condition):
        if self.is_agent is True:
            raise TypeError("We only load experiments into observed trajectories.")
        dir_labels = {
